...
#记录器
logger = logging.getLogger('jointlog')
logger.setLevel(logging.DEBUG)

#处理器handler
fileHandler = logging.FileHandler(filename='joint_log.txt')
fileHandler.setLevel(logging.DEBUG)

#formatter格式
formatter = logging.Formatter("%(asctime)s|%(levelname)8s|%(filename)18s%(lineno)s|%(message)s")
...
def train_m(aug1, aug2, loss_aug_fn, encoder_model, contrast_model, dataloader, optimizer):
    aug1.train()
    aug2.train()
    epoch_loss = 0
...
def main():
    ...
    optimizer = Adam([{'params': aug1.parameters(), 'lr': lr_aug},
    ...
    with tqdm(total=epoch, desc='(T)') as pbar:
        for e in range(1, epoch + 1):
            loss = train_m(aug1, aug2, loss_aug_fn, encoder_model, contrast_model, dataloader, optimizer)
            logger.debug("aug_epoch: {}, loss: {}".format(e, loss))
