...
parser.add_argument('--epoch_aug', dest='epoch_aug', type=int, default=50, help='epoch of aug')
...
parser.add_argument('--aug1', dest='aug1', type=str, default='Feature_Mask', choices=['Origin', 'Node_Drop', 'Feature_Mask'], help='the first augmentation')
parser.add_argument('--aug2', dest='aug2', type=str, default='Node_Drop', choices=['Node_Drop', 'Feature_Mask'], help='the second augmentation')
...
def main():
    ...
    epoch_aug = args.epoch_aug
    epoch = args.epoch
    with tqdm(total=epoch_aug, desc='(T)') as pbar:
        for epoch in range(1, epoch_aug + 1):
            loss_aug = train_m(aug1, aug2, loss_aug_fn, dataloader, optimizer_mask)
            pbar.set_postfix({'loss_aug': loss_aug})
            pbar.update()
